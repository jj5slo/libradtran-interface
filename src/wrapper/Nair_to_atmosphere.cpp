...
//		double GM_E = gm_e_arr[i-1];
		...
		double air_mass_density = get_msis_average_molecular_weight( dt, coord[i-1] , coord[i]) * (atm[i-1].Nair + atm[i].Nair)/2  * 1.0e3;/* [kg m-3] */
...
//		atm[i-1].T = (atm[i-1].p * 1.0e2)/ (atm[i-1].Nair * 1.0e6)  / BOLTZMANN_CONSTANT;/* 状態方程式 */
		//atm[i-1].p = 1013.0;/* TODO TODO TODO uvspecに怒られないようにしているだけ */
		atm[i-1].T = 273.15;/* ぜろど */
		atm[i-1].p = (atm[i-1].T) * (atm[i-1].Nair * 1.0e6) * BOLTZMANN_CONSTANT / 1.0e2;/* 状態方程式 */
	}
	return atm;
}
