...
int main(int argc, char* argv[]){
	...
	double      TOP_PRESS_COEF;
	if(argc == 4){
		REF_PATH       = argv[1];
		PATHS_PATH     = argv[2];
		TOP_PRESS_COEF = std::stod(argv[3]);
	}
	else{
		std::cerr << "Usage: ./min_max REFERENCEatm.dat PATHs.txt TOP_PRESS_COEF" << std::endl;
		return 1;
	}
	...
	for(int file_i = 0; file_i < Nfiles; file_i++){
		...
		pressure_at_top = TOP_PRESS_COEF * pAtm[Nheights-1].p;
			ParamAtmosphere* oldatm = Nair_to_atmosphere_old(Nheights, earth, heights, x, pressure_at_top);
		pressure_at_top = TOP_PRESS_COEF * std::pow(10, (std::log10(pAtm[Nheights-1].p) + std::log10(pAtm[Nheights-2].p))/2.0);
